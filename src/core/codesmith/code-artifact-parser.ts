...
function parseJsonArtifacts(response: string, sourceAgent: string): CodeArtifact[] {
    ...
    for (const item of artifactsRaw) {
        ...
        const updateMode: 'full' | 'unified_diff' = artifact.update_mode === 'unified_diff' ? 'unified_diff' : 'full';
        artifacts.push({
            path: normalizedPath,
...
function dedupeAndValidate(artifacts: CodeArtifact[]): CodeArtifact[] {
    ...
    const output = Array.from(seen.values())
        .map((artifact) => ({
            ...
            update_mode: (artifact.update_mode === 'unified_diff' ? 'unified_diff' : 'full') as 'full' | 'unified_diff'
        }))
