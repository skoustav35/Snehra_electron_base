...
${escapeBoltTags(file.content)}
  ...
  const handleVfsConnect = async (owner: string, repo: string, branch?: string) => {
    setLoading(true);
    setIsDialogOpen(false);
    setSelectedProvider(null);

    try {
      await githubVfsStore.connect(owner, repo, branch || 'main');

      if (importChat) {
        const message: Message = {
          role: 'assistant',
          content: `Connected to **${owner}/${repo}** in Direct GitHub Editor Mode (VFS). 
          
The files are available in the editor but haven't been downloaded entirely to save resources. When you open a file, it is fetched dynamically. All edits will be directly pushed to GitHub.`,
          id: generateId(),
          createdAt: new Date(),
        };

        await importChat(`VFS: ${owner}/${repo}`, [message]);
      }
    } catch (error) {
      console.error('Error connecting to VFS:', error);
      toast.error('Failed to connect to Virtual File System');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-950 rounded-xl shadow-xl border border-bolt-elements-borderColor dark:border-bolt-elements-borderColor w-full max-w-4xl max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 max-h-[calc(90vh-140px)] overflow-y-auto">
              <GitHubRepositorySelector onClone={handleClone} onVfsConnect={handleVfsConnect} />
            </div>
          </div>
        </div>
      )}
