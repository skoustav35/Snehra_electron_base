...
interface GitHubRepositorySelectorProps {
  ...
  onVfsConnect?: (owner: string, repo: string, branch?: string) => void;
  className?: string;
}
...
export function GitHubRepositorySelector({ onClone, onVfsConnect, className }: GitHubRepositorySelectorProps) {
  ...
  const [pendingAction, setPendingAction] = useState<'clone' | 'vfs' | null>(null);
  ...
  const handleVfsConnectRepository = (repo: GitHubRepoInfo) => {
    setSelectedRepo(repo);
    setPendingAction('vfs');
    setIsBranchSelectorOpen(true);
  };

  const handleBranchSelect = (branch: string) => {
    if (pendingAction === 'clone' && onClone && selectedRepo) {
      const cloneUrl = selectedRepo.html_url + '.git';
      onClone(cloneUrl, branch);
    } else if (pendingAction === 'vfs' && onVfsConnect && selectedRepo) {
      onVfsConnect(selectedRepo.owner.login, selectedRepo.name, branch);
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentRepositories.map((repo) => (
              <GitHubRepositoryCard 
                key={repo.id} 
                repo={repo} 
                onClone={onClone ? () => handleCloneRepository(repo) : undefined} 
                onVfsConnect={onVfsConnect ? () => handleVfsConnectRepository(repo) : undefined} 
              />
            ))}
          </div>
