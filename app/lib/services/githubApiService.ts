...
export class GitHubApiServiceClass {
  ...
  /**
   * Get repository tree (recursive or non-recursive)
   */
  ...
  ): Promise<{ sha: string; url: string; tree: Array<{ path: string; mode: string; type: 'blob' | 'tree'; sha: string; size?: number; url: string }> }> {
    return this._makeRequestInternal<any>(
      `/repos/${owner}/${repo}/git/trees/${branch}${recursive ? '?recursive=1' : ''}`,
    );
  }

  /**
   * Get file content and details from GitHub
   */
  async getFileContent(
    ...
    const data = await this._makeRequestInternal<any>(
  ...
  /**
   * Create or update a single file directly
   */
  async createOrUpdateFile(
    ...
    const body: any = {
      message,
      content: typeof window !== 'undefined' ? window.btoa(unescape(encodeURIComponent(content))) : Buffer.from(content).toString('base64'),
    };
    ...
    return this._makeRequestInternal<any>(`/repos/${owner}/${repo}/contents/${path}`, {
      method: 'PUT',
      headers: {
  ...
  /**
   * Create a single commit containing multiple file additions, updates, or deletions
   */
  async commitMultipleFiles(
    owner: string,
    repo: string,
    branch: string = 'main',
    files: Array<{ path: string; content: string | null }>, // content is null for deletion
    message: string,
  ): Promise<string> {
    // 1. Get current branch reference commit SHA
    const refData = await this._makeRequestInternal<any>(`/repos/${owner}/${repo}/git/ref/heads/${branch}`);
    const latestCommitSha = refData.object.sha;

    // 2. Create blobs in parallel for non-deleted files
    const treeItems = await Promise.all(
      files.map(async (file) => {
        if (file.content === null) {
          // File deletion in Git database API: omit sha to delete or set sha to null (if supported),
          // but tree structure requires setting it to null or omitting it.
          // In git database tree API, if you provide path and mode but no sha, or sha: null, it deletes it.
          return {
            path: file.path,
            mode: '100644',
            type: 'blob' as const,
        ...
        const blobData = await this._makeRequestInternal<any>(`/repos/${owner}/${repo}/git/blobs`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    // 3. Create tree
    const treeData = await this._makeRequestInternal<any>(`/repos/${owner}/${repo}/git/trees`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    // 4. Create commit
    const commitData = await this._makeRequestInternal<any>(`/repos/${owner}/${repo}/git/commits`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    // 5. Update reference
    await this._makeRequestInternal<any>(`/repos/${owner}/${repo}/git/refs/heads/${branch}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
      },
