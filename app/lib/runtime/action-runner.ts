...
export class ActionRunner {
  ...
  async #runFileAction(action: ActionState) {
    ...
    // Intercept for VFS Mode
    if (githubVfsStore.isVfsModeActive.get()) {
      await githubVfsStore.saveFile(action.filePath, action.content);
      logger.debug(`File written to VFS ${action.filePath}`);
      return;
    }
    ...
    try {
      ...
      if (uid && chatId) {
        await WorkspaceRepository.saveFileToCloud(uid, chatId, action.filePath, action.content, false);
      }
    } catch (error) {
