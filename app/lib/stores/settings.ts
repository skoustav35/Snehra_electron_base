...
export const updateProviderSettings = (provider: string, config: IProviderConfig) => {
  ...
  // Scrub API key before writing to store
  if (config.settings?.apiKey) {
    delete config.settings.apiKey;
  }
  ...
  // Extra safety scrub
  if (updatedProvider.settings?.apiKey) {
    delete updatedProvider.settings.apiKey;
  }
...
const getInitialSettings = () => {
  ...
  return {
    ...
    codesmithMode: isBrowser ? localStorage.getItem(SETTINGS_KEYS.CODESMITH_MODE) || 'balanced' : 'balanced',
  };
};
...
export const codesmithModeStore = atom<string>(initialSettings.codesmithMode);
...
export const updateCodesmithMode = (mode: string) => {
