...
// Initialize connection from Firestore on startup (requires auth)
async function initializeConnection() {
  try {
    const user = auth.currentUser;
    if (!user) {
      // Clean up any old unsafe local storage
    ...
    const savedConnection = await SettingsRepository.getConnection(user.uid, 'github');

    if (savedConnection) {
      // Ensure tokenType is set
      if (!savedConnection.tokenType) {
        savedConnection.tokenType = 'classic';
      }

      // Only set if we have a valid user
      if (savedConnection.user) {
        githubConnectionAtom.set(savedConnection);
      }
    }
  } catch (error) {
    console.error('Error initializing GitHub connection from Firestore:', error);
  }
}
...
if (typeof window !== 'undefined') {
  auth.onAuthStateChanged((user) => {
    if (user) {
      ...
      githubConnectionAtom.set({ user: null, token: envToken || '', tokenType: 'classic' });
    }
  });
}
...
export const githubConnectionStore = {
  ...
  async connect(token: string, tokenType: 'classic' | 'fine-grained' = 'classic'): Promise<void> {
    ...
    try {
      ...
      // Clean up any old legacy cookies
      Cookies.remove('githubUsername');
      Cookies.remove('githubToken');
      Cookies.remove('git:github.com');
      localStorage.removeItem('github_connection');
      ...
      if (currentUser) {
        await SettingsRepository.setConnection(currentUser.uid, 'github', connection);
      } else {
        throw new Error('Must be logged in to connect GitHub securely.');
      }
  ...
  async disconnect(): Promise<void> {
    ...
    // Clear all legacy GitHub-related cookies
    Cookies.remove('githubToken');
    Cookies.remove('githubUsername');
    Cookies.remove('git:github.com');
    ...
    const currentUser = auth.currentUser;
    if (currentUser) {
      await SettingsRepository.setConnection(currentUser.uid, 'github', null);
    }
