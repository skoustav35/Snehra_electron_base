...
if (typeof window !== 'undefined') {
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      try {
        const savedConnection = await SettingsRepository.getConnection(user.uid, 'vercel');
        if (savedConnection && (savedConnection.user || savedConnection.token)) {
          vercelConnection.set(savedConnection);
        } else if (envToken) {
          // Fallback to env token
          vercelConnection.set({ user: null, token: envToken, stats: undefined });
        }
      } catch (error) {
        console.error('Error initializing Vercel connection from Firestore:', error);
      }
    } else {
...
export const updateVercelConnection = (updates: Partial<VercelConnection>) => {
  ...
  if (user) {
    SettingsRepository.setConnection(user.uid, 'vercel', newState).catch((err) =>
      console.error('Failed to securely save Vercel connection:', err)
    );
  }
