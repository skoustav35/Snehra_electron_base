...
import { WorkspaceRepository } from '~/lib/firebase/repositories/workspaceRepository';
...
export class FilesStore {
  ...
  async saveFile(filePath: string, content: string) {
    ...
    try {
      ...
      this.#syncFileToCloud(filePath, content, false);
  ...
  async #syncFileToCloud(filePath: string, content: string, isBinary: boolean) {
    const uid = auth.currentUser?.uid;
    const chatId = getCurrentChatId();
    if (!uid || !chatId) return;

    try {
      await WorkspaceRepository.saveFileToCloud(uid, chatId, filePath, content, isBinary);
    } catch (error: any) {
      logger.error('Failed to sync file to cloud', error);
      configuredToast.error('Failed to sync file to cloud: ' + error.message);
    }
  }

  async #deleteFileFromCloud(filePath: string) {
    const uid = auth.currentUser?.uid;
    const chatId = getCurrentChatId();
    if (!uid || !chatId) return;

    try {
      await WorkspaceRepository.deleteFileFromCloud(uid, chatId, filePath);
    } catch (error: any) {
      logger.error('Failed to delete file from cloud', error);
      configuredToast.error('Failed to delete file from cloud: ' + error.message);
    }
  }
  ...
  async createFile(filePath: string, content: string | Uint8Array = '') {
    ...
    try {
      ...
      if (isBinary) {
        ...
        this.#syncFileToCloud(filePath, base64Content, true);
      } else {
        ...
        this.#syncFileToCloud(filePath, content as string, false);
      }
  ...
  async deleteFile(filePath: string) {
    ...
    try {
      ...
      this.#deleteFileFromCloud(filePath);
