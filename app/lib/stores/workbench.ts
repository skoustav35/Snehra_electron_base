...
import { githubVfsStore } from './githubVfs';
...
export class WorkbenchStore {
  ...
  unifiedFiles = computed(
    [this.#filesStore.files, githubVfsStore.isVfsModeActive, githubVfsStore.files],
    (localFiles, isVfs, vfsFiles) => {
      return isVfs ? vfsFiles : localFiles;
    }
  );
  ...
  get files() {
    return this.unifiedFiles as any;
  }
  ...
  setSelectedFile(filePath: string | undefined) {
    ...
    if (githubVfsStore.isVfsModeActive.get() && filePath) {
      githubVfsStore.fetchFileContentIfNeeded(filePath);
    }
  }
  ...
  async saveFile(filePath: string) {
    ...
    if (githubVfsStore.isVfsModeActive.get()) {
      await githubVfsStore.saveFile(filePath, document.value);
    } else {
  ...
  async createFile(filePath: string, content: string | Uint8Array = '') {
    try {
      const isVfs = githubVfsStore.isVfsModeActive.get();
      const success = isVfs 
        ? await githubVfsStore.createFile(filePath, content)
  ...
  async createFolder(folderPath: string) {
    try {
      if (githubVfsStore.isVfsModeActive.get()) {
        return await githubVfsStore.createFolder(folderPath);
      }
  ...
  async deleteFile(filePath: string) {
    try {
      ...
      const isVfs = githubVfsStore.isVfsModeActive.get();
      const success = isVfs
        ? await githubVfsStore.deleteFile(filePath)
  ...
  async deleteFolder(folderPath: string) {
    try {
      ...
      const isVfs = githubVfsStore.isVfsModeActive.get();
      const success = isVfs
        ? await githubVfsStore.deleteFolder(folderPath)
        : await this.#filesStore.deleteFolder(folderPath);
