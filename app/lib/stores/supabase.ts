...
if (typeof window !== 'undefined') {
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      try {
        const savedConnection = await SettingsRepository.getConnection(user.uid, 'supabase');
        if (savedConnection && (savedConnection.user || savedConnection.token)) {
          supabaseConnection.set(savedConnection);
        }
      } catch (error) {
        console.error('Error initializing Supabase connection from Firestore:', error);
      }
    } else {
...
export function updateSupabaseConnection(connection: Partial<SupabaseConnectionState>) {
  ...
  if (currentUser) {
    if (connection.user || connection.token || connection.selectedProjectId !== undefined || connection.credentials) {
      SettingsRepository.setConnection(currentUser.uid, 'supabase', newState).catch((err) =>
        console.error('Failed to securely save Supabase connection:', err)
      );
    } else {
      SettingsRepository.setConnection(currentUser.uid, 'supabase', null).catch((err) =>
        console.error('Failed to securely clear Supabase connection:', err)
      );
    }
  }
